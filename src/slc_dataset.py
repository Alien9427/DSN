...
import slc_functions


def read_txt(txt_file, catefile):
    pd_data = pd.read_csv(txt_file)
    catename2label = pd.read_csv(catefile)
    pd_data['label'] = None
...
class SLC_spe_4D(Dataset):
    def __init__(self, txt_file, spe_dir, catefile, spe_transform=None):
        self.data = read_txt(txt_file, catefile)
...
class SLC_img(Dataset):
    def __init__(self, txt_file, root_dir, catefile, transform = None):
        self.data = read_txt(txt_file, catefile)
    ...
    def __getitem__(self, idx):
        ...
        # data = np.log(np.abs(slc_data) + 1) # mstar
...
class SLC_img_spe4D(Dataset):
    def __init__(self, txt_file, img_dir, spe_dir, catefile, img_transform=None, spe_transform=None):
        self.data = read_txt(txt_file, catefile)
    ...
    def __getitem__(self, idx):
        ...
        slc_img = np.log2(np.abs(slc_img) + 1) / 16 # slc data
        # slc_img = np.log(np.abs(slc_img) + 1)  # mstar
